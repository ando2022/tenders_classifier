#!/usr/bin/env python3
"""
FINAL FIX: Consolidate scraped data with proper field mapping
Fixes all non-uniform population issues between SIMAP and EU Tender sources.
"""
import pandas as pd
...
def consolidate_simap_data(simap_file):
    ...
    for _, row in df.iterrows():
        tender = {
            ...
            'original_language': row.get('languages', 'DE'),  # SIMAP: languages field
            ...
            # Procedure data - SIMAP doesn't have these, set to None
            'procedure_type': None,
...
def consolidate_eu_tender_data(eu_file):
    ...
    consolidated = []
    for _, row in df.iterrows():
        # Extract language from language_submission field
        language = row.get('language_submission', 'EN')  # EU Tender: language_submission field
        ...
        tender = {
            ...
            'description_original': row.get('description', ''),
            'original_language': language,  # FIXED: Use language_submission
            ...
            'buyer_country': extract_country(row.get('buyer_country', '')),
...
def extract_country(country_field):
    """Extract country code from EU Tender format like ['BEL'] or ['LUX']"""
    if pd.isna(country_field) or not country_field:
        return ''
    
    # Handle formats like ['BEL'] or ['LUX']
    country_str = str(country_field)
...
def consolidate_all_sources():
    ...
    print("🔄 Consolidating data from all sources (FINAL FIX)...")
    print("=" * 60)
    ...
    # Check data uniformity - SHOULD BE FIXED NOW
    print(f"\n🔍 DATA UNIFORMITY CHECK (SHOULD BE IMPROVED):")
    print("=" * 60)
    ...
    for source in df_consolidated['source'].unique():
        ...
        for col in key_columns:
            ...
            # Show sample values
            if populated > 0:
                sample = source_df[col].dropna().iloc[0]
                print(f"    Sample: {str(sample)[:50]}...")
    
    # Save to CSV
    output_dir = 'mvp/data'
    os.makedirs(output_dir, exist_ok=True)
    output_file = f'{output_dir}/consolidated_tenders_FINAL_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
