#!/usr/bin/env python3
"""
FIXED: Test Data Classification Analysis
Properly compare expected vs predicted results with case-insensitive matching.
"""
import pandas as pd
import sys
...
def run_classification_test():
    """Run classification on test data - FIXED VERSION"""
    
    print("🧪 TEST DATA CLASSIFICATION ANALYSIS (FIXED)")
    print("=" * 80)
    ...
    for idx, row in df.iterrows():
        ...
        is_correct = (prediction.upper() == expected.upper())
        if is_correct:
    ...
    # Save results
    output_file = f'mvp/data/test_classification_results_FIXED_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
if __name__ == "__main__":
    ...
    # Show relevant tenders correctly identified
    relevant_correct = correct_predictions[correct_predictions['expected'] == 'YES']
    if len(relevant_correct) > 0:
        print(f"\n✅ Relevant tenders correctly identified:")
        for _, row in relevant_correct.head(3).iterrows():
            print(f"  - {row['title'][:60]}...")
            print(f"    Confidence: {row['confidence']}%")
    
    # Show non-relevant tenders correctly rejected
    non_relevant_correct = correct_predictions[correct_predictions['expected'] == 'NO']
    if len(non_relevant_correct) > 0:
        print(f"\n✅ Non-relevant tenders correctly rejected:")
        for _, row in non_relevant_correct.head(3).iterrows():
            print(f"  - {row['title'][:60]}...")
            print(f"    Confidence: {row['confidence']}%")
