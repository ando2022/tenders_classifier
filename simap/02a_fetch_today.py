...
today = datetime.now()
pub_date_from = today.strftime("%Y-%m-%d")
pub_date_to = today.strftime("%Y-%m-%d")

print(f"Fetching SIMAP data from {pub_date_from} to {pub_date_to}")
...
if len(result_df) == 0:
    ...
    result_df.to_csv("simap.csv", index=False)
    print("Empty simap.csv created.")
    exit(0)
